...
ShiftRegister gShiftRegister;
bool gShiftRegisterDataPending = false;
// SHIFT REGISTER end
...
void Bela_listhook(const char *source, int argc, t_atom *argv){
	if(strcmp(source, "bela_shiftRegister") == 0){
		bool data[argc];
		for(int n = 0; n < argc; ++n)
		{
			if(!libpd_is_float(&argv[n]))
			{
				rt_fprintf(stderr, "Wrong format for bela_shiftRegister, requires either [setup 1 2 3 32(, or a list of floats\n");
				return;
			}
			data[n] = libpd_get_float(&argv[n]);
		}
		if(gShiftRegisterDataPending)
		{

			rt_printf("bela_shiftRegister: data shifting still in progress, ignoring new data\n");
			return;
		}
...
void Bela_messageHook(const char *source, const char *symbol, int argc, t_atom *argv){
	// SHIFT REGISTER start
	if(strcmp(source, "bela_shiftRegister") == 0){
		if(strcmp(symbol, "setup") == 0){
			char errorMessage[] =  "Wrong format for Bela_gShiftRegister, setup requires 4 floats (data_pin, clock_pin, latch_pin, number_of_bits)\n";
			if(argc != 4)
			{
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int tick = 0; tick < numberOfPdBlocksToProcess; ++tick)
	{
		...
		if(gDigitalEnabled)
		{
			...
			if(gShiftRegisterDataPending)
			{
				for(unsigned int n = digitalFrameBase; n < digitalFrameBase + gLibpdBlockSize; ++n)
				{
					gShiftRegister.render(context, n);
				}
				if(gShiftRegister.dataSent())
				{
					gShiftRegisterDataPending = false;
					libpd_bang("bela_shiftRegisterDone");
				}
			}
			// SHIFT REGISTER end
